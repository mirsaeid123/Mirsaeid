clear all;
tic
% This program has been designed by: Dr. Mirsaeid Hosseini Shirvani
%It needs 9 data sets and different functions
n=25; %number of fog nodes
m=200; %number of tasks
SwarmSize=100;
BW=50; %Mbps Mega bit per second
% Fog specifications%----------
CPU_rate=zeros(1,n); % MIPS
CPU_price=zeros(1,n); %G$/s
Mem_price=zeros(1,n); %G$/MB
BW_price=zeros(1,n); %G$/MB
CPU_fail=zeros(1,n); % failure rate/hour
% Task specification %----------
Inst=zeros(1,m); % *10^9
Mem=zeros(1,m); % MB
File_in=zeros(1,m); % MB
File_out=zeros(1,m); % MB
%---------------------------
%--Reading Datset of processing from Excel files---
CPU_rate=csvread('CPU_rate7.csv');
CPU_price=csvread('CPU_price7.csv');
Mem_price=csvread('Mem_price7.csv');
BW_price=csvread('BW_price7.csv');
CPU_fail=csvread('CPU_fail7.csv');
%----------------------------------

Inst=randi([1,100],1,m);
Mem_request=randi([5,20],1,m)*10;
File_request_in=randi([1,10],1,m)*10;
File_request_out=randi([1,10],1,m)*10;
%--Reading Datset of IoT requirements from Excel files---
Inst=csvread('Inst7.csv');
Mem_request=csvread('Mem_request7.csv');
File_request_in=csvread('File_request_in7.csv');
File_request_out=csvread('File_request_out7.csv');
Swarm=zeros(SwarmSize,m);
Fitness1=zeros(1,SwarmSize); %makespan
Fitness2=zeros(1,SwarmSize); %monetary Cost
Fitness3=zeros(1,SwarmSize); % total SFF

PS_Size=0; %Pareto size
PS=zeros(SwarmSize,m); %for pareto set
%----Initialization-----------
for i=1:SwarmSize
    for j=1:m
      Swarm(i,j)=randi([1,n]);
    end
    Swarm(i,:)=LoadBalancer3( Swarm(i,:), Inst,CPU_rate, m,n,BW, File_request_in,File_request_out );
    Fitness1(i)= Makespan( Swarm(i,:), Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
    Fitness2(i)=Cost( Swarm(i,:), Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
    Fitness3(i)=SFF( Swarm(i,:), Inst, CPU_rate,CPU_fail, m, n );
end

%0----Creating the first Pareto Set
[ Rank,Len,NewSol,SolSize ] = fast_non_dominated_sort( Swarm, SwarmSize,Fitness1,Fitness2,Fitness3 );
%[DSS,LEN4]=CrowdingDistance(NewSol,SolSize,Fitness1,Fitness2,Fitness3);

PS_Size=0;
for i=1:length(Rank(1).front)
    PS_Size=PS_Size+1;
    PS(PS_Size,:)=Swarm(Rank(1).front(i),:);
end

%Calculate three fitness functions for paret solutions
for i=1:PS_Size
    Fit1(i)= Makespan( PS(i,:), Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
    Fit2(i)=Cost( PS(i,:), Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
    Fit3(i)=SFF( PS(i,:), Inst, CPU_rate,CPU_fail, m, n );

end
%------Set weights as objectives improtance in the whole
w1=0.333;
w2=0.333;
w3=0.333;
min1=min(Fitness1);
Max1=max(Fitness1);
min2=min(Fitness2);
Max2=max(Fitness2);
min3=min(Fitness3);
Max3=max(Fitness3);
%ScoreFirst=FinalScore2( Fit1(1),Fit2(1),Fit3(1), w1,w2,w3,min1,Max1,min2,Max2,min3,Max3 )

% find best leaders: alpha, beta, delta wolves
[DSS,LEN4]=CrowdingDistance(PS,PS_Size,Fit1,Fit2,Fit3);
NUM=3; %number of leaders
Leaders=zeros(NUM,m);
INDEX=zeros(1,LEN4);
j=0;
while j~=NUM
    MX=-inf;
    idx=-1;
    for k=1:LEN4
        if (INDEX(k)==0)&&(DSS(k).CD > MX)
            idx=k;
            MX=DSS(k).CD;
        end
    end
    j=j+1;
    Leader(j,:)=DSS(idx).Chr;
    INDEX(idx)=1;
end

%-----Determine Alpha, Beta, and Delta wolves from Pareto set
Alpha=PS(1,:);
Beta=PS(2,:);
Delta=PS(3,:);
F11= Makespan( Alpha, Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
F12=Cost( Alpha, Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
F13=SFF( Alpha, Inst, CPU_rate,CPU_fail, m, n );
F21= Makespan( Beta, Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
F22=Cost( Beta, Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
F23=SFF( Beta, Inst, CPU_rate,CPU_fail, m, n );
F31= Makespan( Delta, Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
F32=Cost( Delta, Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
F33=SFF( Delta, Inst, CPU_rate,CPU_fail, m, n );
%--------Starting the main iteration-------
MaxIter=50;
Iter=1;
while Iter<=MaxIter
    for i=1:SwarmSize
        r=rand;
        if r>0.5
          %Exploitation, use leaders' experiences
          TEMP=Exploitation(Swarm(i,:),Alpha,Beta,Delta,m);
        else
          %Exploration, use one of random explorations
          d=randi([1,4]);
          if d==1
              TEMP=Exploration_Type_1(Swarm(i,:),m);
          elseif d==2
              TEMP=Exploration_Type_2(Swarm(i,:),m);
          elseif d==3
              TEMP=Exploration_Type_3(Swarm(i,:),m);
          else
              TEMP=Exploration_Type_4(Swarm(i,:),m);
          end
        end
        %TEMP=LoadBalancer3( Swarm(i,:), Inst,CPU_rate, m,n,BW, File_request_in,File_request_out );
        FTemp1= Makespan( TEMP, Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
        FTemp2=Cost( TEMP, Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
        FTemp3=SFF( TEMP, Inst, CPU_rate,CPU_fail, m, n );        
        if Dominate(FTemp1,FTemp2,FTemp3,Fitness1(i),Fitness2(i),Fitness3(i))==1
            Swarm(i,:)=TEMP;
            %Caculate fitness of updated wolf------------
            Fitness1(i)= Makespan( Swarm(i,:), Inst, CPU_rate, m, n, BW, File_request_in,File_request_out );
            Fitness2(i)=Cost( Swarm(i,:), Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n );
            Fitness3(i)=SFF( Swarm(i,:), Inst, CPU_rate,CPU_fail, m, n );           
        end
    end
    % Determine new Pareto Set-----------------
    [ Rank,Len,NewSol,SolSize ] = fast_non_dominated_sort( Swarm, SwarmSize,Fitness1,Fitness2,Fitness3 );
    %[DSS,LEN4]=CrowdingDistance(NewSol,SolSize,Fitness1,Fitness2,Fitness3);
    PS_Size=0;
    for i=1:length(Rank(1).front)
      PS_Size=PS_Size+1;
      PS(PS_Size,:)=Swarm(Rank(1).front(i),:);
    end
    %Determine new leaders Alpha,Beta, and Delta
    NUM=3; %number of leaders-------------------
    Leaders=zeros(NUM,m);
    INDEX=zeros(1,LEN4);
    j=0;
    while j~=NUM
        MX=-inf;
        idx=-1;
        for k=1:LEN4
            if (INDEX(k)==0)&&(DSS(k).CD > MX)
                idx=k;
                MX=DSS(k).CD;
            end
        end
        j=j+1;
        Leader(j,:)=DSS(idx).Chr;
        INDEX(idx)=1;
    end
    %---Update new leaders: Alpha, Beta, and Delta wolves
    Alpha=PS(1,:);
    Beta=PS(2,:);
    Delta=PS(3,:);

    if Iter==MaxIter/2
        min1=min(Fitness1);
        Max1=max(Fitness1);
        min2=min(Fitness2);
        Max2=max(Fitness2);
        min3=min(Fitness3);
        Max3=max(Fitness3);
    end
    
    Iter=Iter+1;
end


F11= Makespan( Alpha, Inst, CPU_rate, m, n, BW, File_request_in,File_request_out )
F12=Cost( Alpha, Inst,Mem,File_request_in,File_request_out, CPU_rate,CPU_price,Mem_price,BW_price, m, n )
F13=SFF( Alpha, Inst, CPU_rate,CPU_fail, m, n )
ScoreAlpha=FinalScore2( F11,F12,F13, w1,w2,w3,min1,Max1,min2,Max2,min3,Max3 )

% F11: makespan of the best solution
% F12: ToC of the best solution
% F13: TSSF of the best solution
% ScoreAlpha: cost score of the best solution

ScoreAlpha=FinalScore2( F11,F12,F13, w1,w2,w3,min1,Max1,min2,Max2,min3,Max3 )
toc


